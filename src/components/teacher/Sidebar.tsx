...
export default function Sidebar({ username }: { username?: string }) {
    ...
    const settingsUrl = username
        ? `/profile/${username}?tab=settings`
        : "/profile";

    return (
        <>
            {/* Mobile top bar */}
            <div className="sm:hidden flex items-center justify-between p-3 h-fit">
                <button
                    type="button"
          ...
          bg-[var(--bg-card)] p-4"
                    >
                        ...
                        <div className="mt-6">
                            <Link
                                href={settingsUrl}
                                className={`flex items-center gap-2 px-3 py-2 rounded-md ${pathname === settingsUrl ? "bg-indigo-300" : "hover:bg-muted-20"}`}
                            >
                                <Settings size={18} />
                                <span className="text-sm">Settings</span>
                            </Link>
