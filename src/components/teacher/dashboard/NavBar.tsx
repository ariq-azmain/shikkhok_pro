      ...
      <div className="flex items-center gap-3">
        <Link href={profileUrl} className="flex items-center gap-2">
          {user?.avatar ? (
            ...
            <div className="w-9 h-9 rounded-full bg-indigo-500 flex items-center justify-center text-white font-bold text-sm">{(user?.displayName || "U").split(" ").map((s: string) => s[0]).join("").slice(0, 2)}</div>
          )}
        </Link>
      </div>
    </header>
