  ...
  return (
    <>
      ...
      <div className="sm:hidden flex items-center justify-between p-3">
        <button onClick={() => setOpen(true)} aria-label="Open menu" className="p-2 rounded-md bg-white/3">
          <Menu size={18} />
        </button>
      ...
      <aside className="hidden sm:flex sm:flex-col sm:w-64 sm:pt-4 sm:gap-4">
        <nav className="flex-1 px-4 space-y-2">
          {nav.map((item) => (
            <Link key={item.href} href={item.href} className={`flex items-center gap-3 px-3 py-2 rounded-md ${pathname === item.href ? "bg-indigo-500/10" : "hover:bg-white/5"}`}>
              <item.icon size={18} />
        ...
        <div className="px-4 py-3">
          <a href={settingsUrl} className="flex items-center gap-2 px-3 py-2 rounded-md hover:bg-white/5">
            <Settings size={18} />
        ...
        <div className="fixed inset-0 z-50 bg-black/40 sm:hidden">
          <div className="absolute left-0 top-0 bottom-0 w-72 bg-[var(--bg-card)] p-4">
            <div className="flex items-center justify-between mb-4">
              <div className="font-semibold">Menu</div>
              <button onClick={() => setOpen(false)} className="p-2 rounded-md bg-white/3"><X size={18} /></button>
            </div>
            <nav className="space-y-2">
              {nav.map((item) => (
                <Link key={item.href} href={item.href} onClick={() => setOpen(false)} className="flex items-center gap-3 px-3 py-2 rounded-md hover:bg-white/5">
                  <item.icon size={18} />
                  <span className="text-sm">{item.label}</span>
                </Link>
              ))}
