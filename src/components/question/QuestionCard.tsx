...
export default function QuestionCard({ question }: { question: Question }) {
  ...
  const subjectColor =
    SUBJECT_COLORS[question.subject] ?? "text-slate-400 bg-slate-400/10";
  const totalQs =
    ...
    >
      ...
      <Link href={`/q/${question.id}`} className="block p-5 pb-3">
        ...
        <div className="flex flex-wrap gap-2 mb-4">
          <span
            className={`text-xs font-semibold px-2.5 py-0.5 rounded-full ${subjectColor}`}
          >
            {question.subject}
          </span>
          ...
          <span
            className={`text-xs font-semibold px-2.5 py-0.5 rounded-full border ${difficulty.className}`}
          >
            {difficulty.label}
          </span>
        </div>
