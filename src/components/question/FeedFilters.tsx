...
const SUBJECTS = ["All", "Mathematics", "Physics", "Chemistry", "Biology", "Science", "English", "Bangla", "ICT"];
...
export default function FeedFilters({ filters, onChange }: Props) {
  return (
    <div className="space-y-3">
      {/* Subject */}
      <div>
        <p className="text-xs font-semibold mb-2" style={{ color: "var(--text-muted)" }}>SUBJECT</p>
      ...
      {/* Class */}
      <div>
        <p className="text-xs font-semibold mb-2" style={{ color: "var(--text-muted)" }}>CLASS</p>
        <div className="flex gap-2 overflow-x-auto pb-1 no-scrollbar">
          {CLASSES.map((c) => (
            <FilterPill
              key={c}
      ...
      {/* Difficulty */}
      <div>
        <p className="text-xs font-semibold mb-2" style={{ color: "var(--text-muted)" }}>DIFFICULTY</p>
        <div className="flex gap-2">
          {DIFFICULTIES.map((d) => (
            <FilterPill
              key={d}
              label={d}
              active={filters.difficulty === (d === "All" ? "" : d.toUpperCase())}
              onClick={() => onChange("difficulty", d === "All" ? "" : d.toUpperCase())}
            />
          ))}
        </div>
      </div>
