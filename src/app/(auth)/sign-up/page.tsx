...
function OTPInput({ value, onChange }: { value: string; onChange: (v: string) => void }) {
  ...
  const handleChange = (i: number, v: string) => {
    ...
    arr[i] = v.slice(-1);
    onChange(arr.join(""));
    if (v && i < 5) document.getElementById(`sup-otp-${i + 1}`)?.focus();
  };

  const handleKeyDown = (i: number, e: React.KeyboardEvent) => {
    if (e.key === "Backspace" && !digits[i] && i > 0)
      document.getElementById(`sup-otp-${i - 1}`)?.focus();
  };
        ...
        >
            ...
            <>
              <div className="text-center mb-7">
                ...
                <p className="text-sm" style={{ color: "var(--text-secondary)" }}>
                  Join thousands of teachers & students on Shikkhok Pro
                </p>
              </div>
          ...
          {view === "email_form" && (
            <>
              ...
              <div className="mb-7">
                <h1 className="text-2xl font-bold text-white mb-1.5">Sign up with email</h1>
                <p className="text-sm" style={{ color: "var(--text-secondary)" }}>
          ...
          {view === "verify" && (
            <>
              ...
              <div className="text-center mb-8">
                ...
                <h1 className="text-2xl font-bold text-white mb-1.5">Verify your email</h1>
                <p className="text-sm" style={{ color: "var(--text-secondary)" }}>
              ...
              <div className="flex flex-col gap-5">
                ...
                <button
                  ...
                  style={{ background: "var(--color-primary)", color: "white" }}
                >
                  {loading === "verify" ? <><Spinner /> Verifying…</> : "Verify & Create Account"}
                </button>

                <p className="text-center text-xs" style={{ color: "var(--text-muted)" }}>
                  Didn't receive it?{" "}
                  <button
                    onClick={() => { setOtp(""); signUp.prepareEmailAddressVerification({ strategy: "email_code" }); }}
...
function ErrorMsg({ msg }: { msg: string }) {
  return (
    <div className="flex items-start gap-2 px-3 py-2.5 rounded-xl text-sm text-red-400 bg-red-500/8 border border-red-500/15 mt-4">
